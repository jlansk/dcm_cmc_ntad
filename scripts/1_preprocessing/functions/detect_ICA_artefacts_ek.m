function [Out,ICs] = detect_ICA_artefacts_ek(S);
...
% for example, spm_eeg_montage, to procd /imaging/projects/cbu/ntad/meg_data/C1019/ject out of data topographic patterns 
...
%   TemAbsPval = p-value threshold forcd /imaging/projects/cbu/ntad/meg_data/C1019/ absolute temporal correlation to define artifact (eg 0.01)
...
%         [Out.weights,sphere,compvars,bias,signs,lrates,ICs] = runica(d,'pca',PCA_dim,'extended',1,'maxsteps',800,'stop',0.0001); %EK increased the convergence pt
%     end
    
    [ICs,~,Out.weights] = fastica(d, ...
        'g',                'tanh',...
        'lastEig',          60,...
        'numOfIC',          60,...
        'approach',         'symm',...
...
for r = 1:Nrefs(1)
    ...
    for k = 1:PCA_dim
        [tcor,tempval(r,k)] = corr(refs.tem{r}',ICs(:,k));
        Out.temcor(r,k)=abs(tcor);%EK
    end
        
end
try
    temremove= unique([find(abs(Out.temcor(1,:))>0.3), find(abs(Out.temcor(2,:))>0.3),find(abs(Out.temcor(3,:))>0.3)]);  %EK
catch
    temremove= unique([find(abs(Out.temcor(1,:))>0.3), find(abs(Out.temcor(2,:))>0.3)]);  %EK for data without ECG
end
...
%         for k = 1:PCA_dim
%             [scor,spapval(r,k)] = corr(refs.spa{r}',iweights(:,k));
%             Out.spacor(r,k)=abs(scor);
%         end
%         
...
%sparemove= unique([find(abs(Out.spacor(1,:))>0.7), find(abs(Out.spacor(2,:))>0.7),find(abs(Out.spacor(3,:))>0.7)]);  
sparemove=[];
...
% end
%Out.allrem=temremove;

remove=unique([temremove, sparemove]); %EK
 
if ~isempty(temremove)
